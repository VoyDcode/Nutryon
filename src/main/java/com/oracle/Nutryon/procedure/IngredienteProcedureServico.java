...
@Service
public class IngredienteProcedureServico {
  private final SimpleJdbcCall insertCall, updateCall, deleteCall;

  public IngredienteProcedureServico(JdbcTemplate jdbc) {
  ...
  public long insert(String name, Long categoryId) {
    var out = insertCall.execute(Map.of("p_nome", name, "p_id_categoria", categoryId));
    return ((Number) out.get("p_id_out")).longValue();
  }

  public void update(long id, String name, Long categoryId) {
    updateCall.execute(Map.of("p_id", id, "p_nome", name, "p_id_categoria", categoryId));
  }
